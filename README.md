...
Marioshka — учебный 2D-платформер, созданный на C++.  
...
[Смотреть видео на YouTube](https://www.youtube.com/watch?v=bBPMZvtBOTA)  
...
## Проблемы и решения
**Проблемы:**
- Игра плохо адаптировалась под разные размеры окна, что приводило к искажению графики или неправильной области отображения.
- Персонаж (Марио) часто застревал на углах платформ и стен, когда двигался или прыгал. Из-за этого его движение было прерывистым.
- В методе `Physics::Update(float deltaTime)` в файле `Physics.cpp` каждый кадр создавался новый объект класса MyGlobalContactListener и устанавливался как слушатель столкновений в мире Box2D:
   ```bash
   world->SetContactListener(new MyGlobalContactListener());
Это приводило к утечкам памяти и потенциальным ошибкам, так как старые слушатели не удалялись, а их количество постоянно росло.
- Сильно взаимозависимый и неразделённый код затруднял расширение функционала и поддержку проекта.

**Решения:**
- Camera учитывает текущий размер окна при позиционировании и отрисовке, что позволяет гибко масштабировать и правильно отображать игру на любых разрешениях.
- Для персонажа используются два округлых элемента — сверху и снизу тела. Они помогают мягко взаимодействовать с углами и наклонными поверхностями, снижая риск застревания и обеспечивая плавное движение. Основная прямоугольная форма задаёт точные границы тела для столкновений с платформами и стенами.
- Объект `MyGlobalContactListener` создаётся один раз при инициализации физического мира в `Physics::Init()` и используется на протяжении всей работы. Такой подход предотвращает утечки памяти и исключает ненужные повторные выделения, обеспечивая корректную работу обработчика столкновений. Это повышает стабильность и производительность физического движка.
- Код структурирован по классам с чётким разграничением ответственности, что облегчает поддержку и развитие проекта.
